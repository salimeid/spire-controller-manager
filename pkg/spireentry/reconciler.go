...
func computeObjectHash(obj any) (string, error) {
...
func computeEndpointsRV(items []corev1.Endpoints) string { //nolint:staticcheck
	var sb strings.Builder
	for i, item := range items {
		if i > 0 {
			sb.WriteByte(',')
		}
		sb.WriteString(item.ResourceVersion)
	}
	return sb.String()
}
...
func (r *entryReconciler) renderPodEntry(ctx context.Context, spec *spirev1alpha1.ParsedClusterSPIFFEIDSpec, pod *corev1.Pod, specHash string, nodeMap map[string]*corev1.Node) (*spireapi.Entry, error) {
	...
	if r.renderCache != nil {
		...
		// 3. Cache the entry
		r.renderCache.Add(podEntryCacheKey(pod.UID), &cachedEntry{
			specHash:    specHash,
